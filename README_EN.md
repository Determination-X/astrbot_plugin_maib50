...
Users can submit `/mai alias submit ieo INFiNiTE ENERZY -Overdoze-`. Admins may review with `/mai alias pending`, `/mai alias approve <id>`, `/mai alias reject <id>`, add directly with `/mai alias add <alias> <full song title>` or delete with `/mai alias del <alias>`. Multiword titles work without quotes; **multiword aliases must be double-quoted**, for example `/mai alias add "infinite energy overdose" INFiNiTE ENERZY -Overdoze-`. Apostrophes inside aliases and song titles (such as `don't` and `DON'T STOP ROCKIN'`) are literal and need no escaping. Song titles are matched literally first, including surrounding quote characters, then retried without paired outer quotes if not found. Changes made through commands take effect immediately with no plugin reload; manually editing the JSON file still requires a reload.
