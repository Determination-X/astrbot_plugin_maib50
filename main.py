...
MUNET munet MuNET""")
    ...
    async def mai_ap50(self, event: AstrMessageEvent):
        ...
        yield event.plain_result("正在查询 AP50 数据，请稍候喵~")
        login_url = "https://lng-tgk-aime-gw.am-all.net/common_auth/login/sid"
        login_data = {"retention": "1", "sid": bot_sid, "password": bot_password}
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/146.0.0.0 Safari/537.36",
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                needs_login = True
                logger.debug(
                    "Starting /mai ap50 lookup for friend_code=%s", friend_code
                )

                if cached_cookies:
                    session.cookie_jar._cookies = cached_cookies  # pyright: ignore[reportAttributeAccessIssue]
                    logger.debug("Loaded cached cookies for /mai ap50 lookup")
                ...
                if needs_login:
                    logger.info("Executing maimai login flow for /mai ap50")
                ...
                if profile is None:
                    ...
                    return
                logger.info(
                    "Parsed %s charts total for AP50 (%s AP charts)",
                    len(entries),
                    sum(self.ap50_helper._is_all_perfect(entry) for entry in entries),
                )

            except Exception as e:
                logger.error("/mai ap50 failed: %s", e, exc_info=True)
                yield event.plain_result(f"登录出错: {str(e)}")
                return

        try:
            image_url = await self.ap50_helper.generate_image(
                self, profile, entries, qq_id
            )
            chain = [Comp.Plain("这是你的AP50数据喵~"), Comp.Image.fromURL(image_url)]
            yield event.chain_result(chain)
        except Exception as e:
